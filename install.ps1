# ccstats installer (Windows / PowerShell) — builds the binary, copies tips
...
$CcstatsDir = Join-Path $ClaudeDir 'ccstats'
...
Write-Output '==> Building ccstats...'
Push-Location $Here
try {
    & go build -o ccstats.exe ./cmd/ccstats
...
Move-Item -Path (Join-Path $Here 'ccstats.exe') -Destination (Join-Path $BinDir 'ccstats.exe') -Force
Write-Output "    installed to $BinDir\ccstats.exe"
